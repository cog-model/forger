...
from policy.tf1_models import OldDuelingModel, OldClassicCnn
...
def get_network_builder(name):
    ...
    Usage Example:
    -------------
    from policy.model import register
...
def make_model(name, obs_space, action_space, reg=1e-5):
    pov = tf.keras.Input(shape=obs_space.shape)
    normalized_pov = pov / 255
    pov_base = OldClassicCnn([32, 64, 64], [8, 4, 3], [4, 2, 1], reg=reg)(normalized_pov)
    head = OldDuelingModel(action_space.n, reg=reg)(pov_base)
    model = tf.keras.Model(inputs={'pov': pov}, outputs=head, name=name)
    return model
