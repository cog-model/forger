...
class TreechopLoader:
    ...
    def sarsd_iter(self, *args, **kwargs):
        for s, a, r, _, d in self.data.batch_iter(*args, **kwargs): #sarsd_iter
            if np.sum(r) < self.threshold:  #original code is sum(r) < self.threshold:
                continue
            ...
            if len(s['pov']) ==1:
                observation = s['pov'][0] #Treechop
            else:
                observation = s['pov'] #Obtain
