...
class ReplayBuffer(object):
    ...
    def _encode_sample(self, idxes):
        ...
        i = idxes[0]
        fake_data = self._storage[i]

        #print('fake data keys', fake_data.keys())

        #print('state',fake_data['state'])

        #print('envdict',self.env_dict)
...
class AggregatedBuff:
    def __init__(self, size, env_dict, episodes_to_decay=50, min_demo_proportion=0.0, cpp=False,
                 **kwargs):
        if cpp:
            buffer_base = cppPER
        else:
        ...
        self._maxsize = size
        self.demo_kwargs = {'size': size, 'env_dict': env_dict, 'eps': 1.0, **kwargs}
        self.demo_buff = buffer_base(size=size, env_dict=env_dict, eps=1.0, **kwargs)
        self.replay_buff = buffer_base(size=size, env_dict=env_dict, eps=1e-3, **kwargs)
